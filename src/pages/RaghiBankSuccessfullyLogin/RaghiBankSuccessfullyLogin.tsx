...
const RaghiBankSuccessfullyLogin: React.FC = () => {
    ...
    return (
        <>
            <Box style={{ flexDirection: 'column', justifyContent: 'space-between', alignSelf: 'center', alignItems: 'center', height: '100vh' }}
                display={'flex'}
                sx={{
                    width: { xs: '100%', md: '100%' },
                ...
                <Box style={{ flexDirection: 'column', alignItems: 'center', alignSelf: 'center' }}
                    ...
                    <Typography variant="body2" color="#9F9F9F" fontWeight={'bold'} fontSize={'14px'} style={{ marginTop: 23, textAlign: 'center' }}>
                        {"PASP Authenticated, We are securely transferring you"}<br />
                        {"to SIDF."}
                    </Typography>
                </Box>
            ...
            {showLoading && <Box style={{ width: '100%', height: '100vh', position: 'absolute', top: 0, left: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 9999, alignContent: 'center', alignItems: 'center', justifyContent: 'center' }}
                display={'flex'}>
