...
const ConsentDetails: React.FC<{ calculated: boolean }> = ({ calculated }) => {
    ...
    function List() {
        if (accounts.length === 0) return <Box sx={{ width: '100%', textAlign: 'center', marginTop: '20px' }}>
            <Box
                component="img"
                loading="lazy"
                sx={{
        ...
        return accounts.map((account: any, index: number) => {
            return (
                ...
                >
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", flexDirection: 'row', width: '100%', height: '100%', padding: '35px 20px' }}>
                        <Box
                            sx={{
                                flexDirection: 'row',
                                display: 'flex',
    ...
    return (
        <Layout>
            ...
            >
                ...
                {loading ? <Spinner /> : <Box sx={{ borderRadius: '27px', display: 'flex', flexDirection: 'column', width: '100%' }}>
                    <List />
                </Box>
                }
            </Box>
